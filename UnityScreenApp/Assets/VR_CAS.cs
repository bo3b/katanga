...
namespace CAS.Utils {
    ...
    [AddComponentMenu("CAS/CAS.Sharpen", -1)]

public class VR_CAS : MonoBehaviour {
	[HideInInspector]
	...
	public CAS.Utils.SharpenType sharpenType = CAS.Utils.SharpenType.Standard;
	...
	void OnEnable()
	{
		...
		if(!m_Shader)
		{
			m_Shader = Shader.Find("Unlit/VR_CAS_Color");
			
			if(!m_Shader)
			{
				Debug.LogError("Couldn't find shader for CAS Sharpen! You shouldn't see this error.");
			}
		}
	}
	...
	protected bool CreateMaterials()
	{
		if (m_Material == null && m_Shader != null && m_Shader.isSupported)
		{
			...
			//Debug.LogError("(1) CAS is not supported on this platform, or you have a shader compilation error somewhere.");
